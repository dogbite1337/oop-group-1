<template>
        <img class="sliderBackground" :src="`src\\projectImages\\${
    ...
    <div  class="SlideShowDiv">
      ...
      <div v-if="currentSliderImageIndex > 0" @click="currentSliderImageIndex=0" class="FirstWhiteCircle" />
      <div class="SpaceBlock" />
      <img v-if="currentSliderImageIndex == 1" class="pacManIcon" src="../../projectImages/cleaned_pacman.png" />
      <div v-if="currentSliderImageIndex != 1" @click="currentSliderImageIndex=1" class="WhiteCircle" />
      <div class="SpaceBlock" />
      <img v-if="currentSliderImageIndex == 2" class="lastPacManIcon" src="../../projectImages/cleaned_pacman.png" />
      <div v-if="currentSliderImageIndex != 2" @click="currentSliderImageIndex=2" class="WhiteCircle" /></div>
</template>
<script>
...
export default {
  ...
  methods: {
      setSliderInterval(){
           this.sliderInterval = setInterval(this.sliderIndexLoop, 5000);
      },

      sliderIndexLoop(){
          if(this.currentSliderImageIndex + 1 >= this.sliderImageURls.length){
